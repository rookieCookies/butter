...
impl UserData for Physics {
    fn add_methods<M: mlua::UserDataMethods<Self>>(methods: &mut M) {
        methods.add_function("delete_collider", |_, collider: ColliderId| {
            Engine::generate().get_mut().scene_manager.physics.delete_collider(collider);
            Ok(())
        });
       ...
       methods.add_function("create_rect_collider", |lua, (node, width, height): (NodeUserData, f32, f32)| {
        ...
        methods.add_function("create_dynamic_rigidbody", |lua, node: NodeUserData| {
            let userdata = Engine::generate().get_mut().scene_manager.physics.create_dynamic_rigidbody(lua, node.0).1;
            Ok(userdata)
        });

        methods.add_function("create_static_rigidbody", |lua, _: ()| {
            let userdata = Engine::generate().get_mut().scene_manager.physics.create_static_rigidbody(lua).1;
            Ok(userdata)
        });

        methods.add_function("create_kinematic_rigidbody", |lua, node: NodeUserData| {
            let userdata = Engine::generate().get_mut().scene_manager.physics.create_kinematic_rigidbody(lua, node.0).1;
            Ok(userdata)
        });

        methods.add_function("attach_collider_to_rigidbody", |_, (cl, rb): (ColliderId, RigidBodyId)| {
            Engine::generate().get_mut().scene_manager.physics.attach_collider_to_rigidbody(cl, rb);
            Ok(())
        });

        methods.add_function("attach_collider_event", |_, (cl, func): (ColliderId, Function)| {
            Engine::generate().get_mut().scene_manager.physics.attach_collider_event(cl, func);
            Ok(())
        });
    }
}
