...
use crate::{engine::Engine, math::vector::Vec3, scene_manager::{node::NodeProperties, scene_template::{TemplateComponent, TemplateComponents, TemplateNode, TemplateNodeId, TemplateScene}}, script_manager::{fields::{Field, FieldValue}, ScriptManager}};
...
impl TemplateComponents {
    pub fn from_table(engine: &mut Engine, table: &toml::Table) -> Option<Self> {
        ...
        for (name, fields) in table.iter() {
            ...
            let script_id = ScriptManager::from_path(engine, &name);
            ...
            for field in script.default_fields.iter() {
                ...
                let Some(field_value) = FieldValue::from_toml(value)
                else {
                    warn!("'{value}' is an unsupported toml value");
                    fields.push(field.1.value.clone());
                    continue;
                };
...
impl FieldValue {
    pub fn from_toml(value: &toml::Value) -> Option<Self> {
        let value = match value {
            toml::Value::String(v) => mlua::Value::String(Engine::lua().create_string(v).unwrap()),
            toml::Value::Integer(v) => mlua::Value::Integer(*v as i32),
            ...
            toml::Value::Table(map) => {
                let is_table = map.get("is_table").map(|x| x.as_bool()).flatten()
                    .unwrap_or(false);
                if !is_table {
                    let x = map.get("x")
                        .map(|x| x.as_float())
                    ...
                    let vec = mlua::Vector::new(x as f32, y as f32, z as f32);
                    mlua::Value::Vector(vec)
                } else { todo!() }
            },
        };
